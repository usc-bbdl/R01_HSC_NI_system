...
from generate_from_file import gen as gen_file
...
#            (filepath, filename) = os.path.split(eachBitFile) 
    ...
    def individualWireIn(self, whichCh, value = -1):
        if value == -1: 
            value = self.allUserInput[whichCh].doubleSpinBox.value()         
        #self.allUserInput[whichCh].doubleSpinBox.setValue(value)
        self.tellFpga(whichCh, value)
        #self.tellWhichFpga(0, whichCh, value)
        print "board",  whichCh, " is now ", value
    ...
    def plotData(self, data):
        ...
        dim = np.shape(data)
        if (data != [] and self.gamma_sta != []):
            #bit_mask = 0x0000001
            forplot = np.array(data)
            #forplot[:,0] = bit_mask & int(forplot[:,0])
            i = 0
            #titl = []
            for eachName, eachChan in self.allFpgaOutput.iteritems():
                #titl = [titl, eachName]
                print eachName
                #subplot(dim[1], 1, i+1)
                #textArray = np.append(textArray, forplot[:, i])
                #plot(forplot[:, i])
                #title(eachName)
                i = i + 1
                #
            path = "C:/nerf_sangerlab/source/py/multC_tester_experiment/1125_resimulation/"
            file_name = "re_expt_rampnhold.gd_" + str(self.gamma_dyn) + ".gs_" + str(self.gamma_sta) + ".rep_" + str(self.rep) + ".dev_fpga_resampled.txt"
            #print titl
            np.savetxt(path + file_name, forplot)
            #show()
            #textArray.shape
            #np.savetxt('test.txt', textArray)
...
#            pipeInData = gen_ramp(T = [0.0, 0.1, 0.101, 0.121, 0.122, 1.0, 1.1, 1.101,  1.121, 1.122, 2.0], L = [1.2, 1.2, 1.5, 1.5, 1.2, 1.2, 1.2, 1.5, 1.5, 1.2, 1.2], FILT = False)   # 20ms pulse for LLSR
            ...
            pipeInData, self.gamma_dyn, self.gamma_sta, self.rep = gen_file()
